...
Appfolio = {
  Listing: function (config) {
    ...
    if (defaultConfig.propertyGroup) {
      listingUrl +=
        '&' +
        escape('filters[property_list]') +
        '=' +
        escape(defaultConfig.propertyGroup)
    }
    if (defaultConfig.themeColor) {
      listingUrl +=
        '&' + escape('theme_color') + '=' + escape(defaultConfig.themeColor)
    }
    if (defaultConfig.themeFont) {
      listingUrl +=
        '&' + escape('theme_font') + '=' + escape(defaultConfig.themeFont)
    }
    if (defaultConfig.defaultOrder) {
      listingUrl +=
        '&' +
        escape('filters[order_by]') +
        '=' +
        escape(defaultConfig.defaultOrder)
    }
    ...
    if (defaultConfig.height != 'auto') {
      document.write(
        "<iframe title='Available Properties' src='" +
          listingUrl +
          "' style='" +
          iFrameStyle +
          "'> </iframe>"
      ...
      var iFrameId =
        'af_iframe_' + document.getElementsByTagName('iframe').length
